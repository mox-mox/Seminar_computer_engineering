...
The internal bus system uses TileLink, which is a bus specific to RocketChip and connects the tiles, caches and other core components. In standard configuration, it consists of two full-duplex, 64-bit data paths.
Using the TileLink and few ancilliarey signals as the only interface for each tile allows easy exchangeing of tiles, which is one of the key features allowing the BOOM project to use the RocketChip to create a full processor around their core. See \cite{rcgen-tutorial} for an overview on how to implement a new tile.
