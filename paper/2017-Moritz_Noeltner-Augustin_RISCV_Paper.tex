...
From a hardware point of view, AXI uses a multi-channel interface where each channel is unidirectional. Signals pertaining to a channel are marked by a common prefix \footnote{In the port list of the RocketChip Verilog module, this is actually in the middle of the name, as the different AXI interfaces are also indicated with a prefix.}. These prefixes are:
...
\end{description}
%}}}
Each channel has handshake signals (\texttt{Valid}, \texttt{Ready} and \texttt{Last} for the two data channels) 

Bus topologies are supported as are multilayered point-to-point connections and mixtures with multiple data- but a shared address channel.
